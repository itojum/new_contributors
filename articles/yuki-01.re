= Agent Coreで初心者でもかんたんAX推進 〜動かして、見て、改善するAIエージェント開発〜
...
生成AIやAIエージェントは、うまく使えれば社内業務を大きく改善できる可能性があります。一方で、「とりあえず動いた」だけでは、会社の中で継続的に使えるサービスにはなかなか近づきません。

LLMを使ったアプリケーションは、従来の決定的なプログラムとは少し違います。同じような入力でも、文脈やモデルの判断によって出力が変わることがあります。つまり、LLMには不確実性があります。

だからこそ、AIエージェントを会社で使える形に近づけるには、「一度作って終わり」ではなく、動かして、観測して、改善する考え方が重要になります。

=== この章の目的

この章の目的は、AWSのサービスであるAmazon Bedrock AgentCoreを使ったエージェント開発の全体像をつかみ、社内で使えるサービスに近づけるための技術的な流れを理解することです。
 ...
 * 生成AIエージェントを業務で使ってみたい人
 * AgentCore Harnessに興味がある人
 * デモではなく、社内で継続的に使える形に近づけたい人
 ...
 * AgentCore各サービスの深い仕様
 * アプリケーション側の詳細な実装
 * 本番運用に必要なセキュリティ、権限管理、ネットワーク設計

AgentCoreのすべての機能を網羅することは目指しません。まずは短い時間で全体像をつかみ、「動かす」「見る」「改善する」という流れを体験することを目標にします。
...
だからこそ、AI Agentを会社で使える形に近づけるには、実行結果を観測し、失敗や改善点を見つけられる状態にしておくことが重要です。
...
業務で使う場合、この不確実性が失敗につながることがあります。そのため、実行結果を確認し、問題を見つけ、小さく直すサイクルが必要です。

しかし、何が起きたのか分からないままでは、どこを直せばよいのか判断できません。まずは改善の材料を集めるため、「見る」ことにフォーカスしていきましょう。

=== AgentCore Observabilityとは
...
ここまでで、Agentを動かし、Observabilityで実行を観測し、Evaluationsで結果を評価する流れを見てきました。このセクションではこれまでで集めた情報をもとに改善をするための方法を学びます。

=== AgentCore Optimizationとは

AgentCore Optimizationは、Agentの実行結果をもとに改善案を作るための機能です。

OptimizationはAgentの実行トレースや評価結果をもとに、どこを変えるとよさそうかを考えます。つまり、ObservabilityとEvaluationsで集めた情報が、Optimizationの材料になります。

トレースには、Agentが実行中に何をしたのかが残っていることを前のセクションで確認できてると思います。Evaluatorには、何をよい結果とみなすかの基準があります。このふたつを組み合わせることで、「どのような実行があり」「何が足りなかったのか」をもとに改善を考えられます。

=== 改善案を確認する

改善案が出てきたら、まず内容を確認します。

ここで大事なのは、提案された内容をそのまま正解として扱わないことです。Optimizationは改善の候補を出してくれますが、最終的に採用するかどうかは、Agentに任せたい業務や制約に照らして判断します。

==== システムプロンプトの改善案

システムプロンプトは、Agentの役割やふるまいを決める重要な部分です。

回答のトーン、優先する判断基準、聞き返しの条件、扱ってはいけない範囲などは、システムプロンプトに表れやすい要素です。応答が期待とずれている場合、システムプロンプトの改善案が出てくることがあります。

==== ツール説明の改善案

ツールを使うAgentでは、ツール説明も改善対象になります。

Agentは、ツールの名前や説明を手がかりに、どの場面でどのツールを使うかを判断します。説明が曖昧だと、必要な場面で使えなかったり、逆に不要な場面で使ってしまったりします。

ツール説明の改善は、Agentの判断を安定させるうえで重要です。

==== 提案をそのまま採用しない

改善案は、必ず人間が確認してから採用します。

特に社内業務で使うAgentでは、セキュリティ、権限、業務ルール、責任範囲を無視できません。スコアを上げるための変更が、業務上は望ましくないこともあります。

Optimizationは、判断を置き換えるものではなく、判断の材料を増やすものとして扱うのがよいでしょう。

=== 改善してもう一度動かす

改善案を確認したら、実際に反映してもう一度Agentを動かします。

このときも、大きく変えすぎないことが大切です。複数の変更を同時に入れると、どの変更が効いたのか分かりづらくなります。

==== プロンプトを更新する

まずは、プロンプトや指示を小さく更新します。

たとえば、回答形式を明確にする、判断基準を追加する、迷ったときに聞き返す条件を書く、といった変更です。小さな変更でも、Agentのふるまいは大きく変わることがあります。

==== Agentを再実行する

変更したら、同じ入力や似た入力でAgentを再実行します。

改善前と改善後で入力条件が大きく変わっていると、結果を比べにくくなります。まずは、問題が起きた入力に近いものを使って、狙った改善ができているかを見ます。

==== もう一度観測・評価する

再実行したら、もう一度ObservabilityとEvaluationsで確認します。

最終応答だけでなく、途中の流れも見ます。ツール呼び出しが変わったのか、実行時間が変わったのか、評価スコアがどう変わったのかを確認します。

ここまで見てはじめて、改善が効いたかどうかを判断できます。

=== 改善を繰り返す

AI Agentの改善は、一回で終わるものではありません。

使う人が増え、入力の種類が増え、業務の前提が変わると、Agentに求められるふるまいも変わります。そのため、動かす、見る、改善するというサイクルを短く回せることが大切です。

==== 改善した内容を再利用する

一度うまくいった改善は、次のAgentでも使えることがあります。

たとえば、聞き返しのルール、回答フォーマット、ツール説明の書き方、評価基準の作り方などは、別のユースケースにも応用できます。改善の知見を小さく残しておくと、次にAgentを作るときの出発点になります。

==== 小さなユースケースを増やす

最初から大きな業務全体をAgentに任せるのではなく、小さなユースケースを少しずつ増やすのが現実的です。

ひとつの業務で動かし、見て、改善する。その結果をもとに、次の入力パターンや関連する業務に広げていきます。こうすると、失敗したときの影響を抑えながら、Agentの使いどころを探せます。

==== 動かす・見る・改善するを繰り返す

この章で扱っている流れは、とても単純です。

まず動かす。次に見る。見えた結果から改善する。そして、もう一度動かす。

単純ですが、AI Agentを業務で使える形に近づけるうえでは、この繰り返しが強力です。LLMの不確実性をなくすことはできません。しかし、不確実性を観測し、評価し、改善することはできます。
