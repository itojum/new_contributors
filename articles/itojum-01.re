= Rubyのパターンマッチをおすすめしたい！〜複雑な条件・抽出のコードを綺麗に！〜
== はじめに
「このコード、もっと綺麗に書けないかな」
...
これによって、深いネスト構造を持ったオブジェクトから簡潔に値の探索・抽出が行えます。例えば、APIのレスポンスのような複雑でネストの深いオブジェクトからデータを抽出するときやOptionモナドなどによってラップされたオブジェクトに処理を行う場合に便利です。
...
== パターン図鑑
パターンマッチで使えるパターンは複数あります。ここでは例を交えながらご紹介します。ここで紹介するパターンをマスターすることで、あらゆるオブジェクトにパターンマッチを使えるようになります。
